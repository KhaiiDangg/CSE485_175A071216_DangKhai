    <!--Footer-->
    <div class="footer">
        <div class="container">
            <div class="container"> 
                <div class="row">
                <div class="col-md-3 footer-mid ">
                    <ul class="nav-link">
                        <a class="nav-link" href="#"><li>Giới thiệu</li></a>
                        <a class="nav-link" href="#"><li>Logo của Khoa CNTT</li></a>
                        <a class="nav-link" href="#"><li>Lời chào mừng</li></a>
                        <a class="nav-link" href="#"><li>Tổ chức</li></a>
                        <a class="nav-link" href="#"><li>Hợp tác liên kết</li></a>
                        
                </ul>
                </div>
                <div class="col-md-3 footer-mid">
                    <ul class="nav-link">
                        <a class="nav-link" href="#"><li>Đào tạo</li></a>
                        <a class="nav-link" href="#"><li>Mô hình đào tạo</li></a>
                        <a class="nav-link" href="#"><li>Đào tọa đại học</li></a>
                        <a class="nav-link" href="#"><li>Định hướng ngành nghề</li></a>
                        <a class="nav-link" href="#"><li>Đào tạo sau đại học</li></a>
                        
                </ul>
                </div>
                <div class="col-md-3 footer-mid">
                    <ul class="nav-link">
                        <a class="nav-link" href="#"><li>Nghiên cứu khoa học</li></a>
                        <a class="nav-link" href="#"><li>Thông tin seminar</li></a>
                        <a class="nav-link" href="#"><li>Các đề tài, dự án</li></a>
                        <a class="nav-link" href="#"><li>Công trình công bố</li></a>
                        <a class="nav-link" href="#"><li>Các phòng thí nghiệm</li></a>
                        
                </ul>
                </div>
                <div class="col-md-3 footer-mid">
                    <ul class="nav-link">
                            <a class="nav-link" href="#"><li>Công nghệ phần mềm</li></a>
                            <a class="nav-link" href="#"><li>Hệ thống thông tin</li></a>
                            <a class="nav-link" href="#"><li>Khoa học máy tính</li></a>
                            <a class="nav-link" href="#"><li>Kỹ thuật máy tính và mạng</li></a>
                            <a class="nav-link" href="#"><li>Toán học</li></a>
                            <a class="nav-link" href="#"><li>Trung tâm tin học</li></a>
                    </ul>
                </div>
            </div></div>
            
        </div>
            <div class="bottom-content" ">
                <p>© 2017 Khoa Công nghệ thông tin - Đại học Thủy lợi<br>
                    Địa chỉ: nhà C1, Đại học Thủy lợi, 175 Tây Sơn, Đống Đa, Hà Nội. Điện thoại: (+84)-024&nbsp;3 5632211. Email: <a class="ui-link-white" href="#">vpkcntt@tlu.edu.vn</a></p>
            </div>
        </div>
    
    </div>